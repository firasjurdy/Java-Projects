...
public class ASCUPDATE {
	...
	public void runQuery( String WID, String unit, String cond, String flag, String EXTJRN ) {
		...
		try {
			...
			Line = br.readLine();
			while( Line != null ){
				rs = s.executeQuery("select CAST(SCAB AS CHAR(4) CCSID 37) as SCAB, CAST(SCAN AS CHAR(6) CCSID 37) as SCAN, CAST(SCAS AS CHAR(3) CCSID 37) as SCAS from kfil" + unit + ".scpf where SCACT = '" + Line + "'");
	...
	public void UpdateJournal( String WID, String Branch, String Primary_Number, String Suffix, String unit, int index, String flag, String EXTJRN, String Internal ){
		try {
			...
			if (Internal.equals("N")){
				if( flag.equals("Y") ){
					s.executeUpdate("insert into " + EXTJRN + ".GYPF values('" + WID + "', '" + WID + "', " + day + ", " + time + ", " + SeqNum + ", 'H35M', 'M', '', '', '', 'ASC', '', '', '', '', 0, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', 0, 0, 0, '', '', '', 'Y', '', 0, 0, '', 0, 0, '', '', '', '', '')");
				...
				if( flag.equals("Y") ){
					s.executeUpdate("insert into " + EXTJRN + ".GYPF values('" + WID + "', '" + WID + "', " + day + ", " + time + ", " + SeqNum + ", 'H35M', 'M', '', '', '', 'ISC', '', '', '', '', 0, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', 0, 0, 0, '', '', '', 'Y', '', 0, 0, '', 0, 0, '', '', '', '', '')");
